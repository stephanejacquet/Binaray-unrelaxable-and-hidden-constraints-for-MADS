...
namespace NOMAD {
  ...
  public:
    ...
    */
    ...
    NOMAD::Double estimate_binary_constraint (const NOMAD::Point  & x, int index) const;
    
    	/// Compute the estimation value of the unrelaxable constraints
    
    NOMAD::Double estimate_unrelaxable_constraints (const NOMAD::Point  & x) const;
    
    ///make a copy of the cache
    
    std::set<const NOMAD::Eval_Point*> copy_the_cache (void) const;
    
    /// leave-one=out cross-validation of the epanechnikov kernel
    
    int leave_one_out_cross_validation_epanechnikov (std::set<const NOMAD::Eval_Point*> my_cache, int index) const;
    
    int leave_one_out_cross_validation_epanechnikov_unrelaxable_constraints (std::set<const NOMAD::Eval_Point*> my_cache) const;
