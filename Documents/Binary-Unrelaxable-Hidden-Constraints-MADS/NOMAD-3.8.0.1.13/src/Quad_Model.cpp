...
{   
    ...
    NOMAD::Double       valeur=estimate_unrelaxable_constraints(x); 
    NOMAD::Double       valeur_bc;
    int           m  = static_cast<int>(_bbot.size());
    NOMAD::Double bboi;
    
    //NOMAD::Double estimation_unrelaxable_constraints=estimate_unrelaxable_constraints (x);
    //cout << "estimation_unrelaxable_constraints= "<<estimation_unrelaxable_constraints<< "\n";
    for ( int i = 0 ; i < m ; ++i )
    {
        
        if ( _alpha[i] )
        {   // We need the ponderation of the i-th output.
            ponderation=0.0;
            const NOMAD::Eval_Point * cur = _cache.begin();
            while (cur)
            {
            ...
            if (ponderation>0 && _bbot[i] != NOMAD::OBJ ){bboi = eval ( x , *_alpha[i] )/ponderation;}
            else{
            ...
            if ( bboi.is_defined() )
            {
                ...
                else if ( _bbot[i] == NOMAD::BC )
                { //cout << h<< " ";
                    valeur_bc=estimate_binary_constraint(x, i);
		     if (valeur>valeur_bc) { valeur_bc=valeur;}
		    h+=valeur_bc*valeur_bc;
                //cout << h<< " \n";
                }
    ...
    //cout << "h=" << h << " \n"; 
    //h+=estimate_unrelaxable_constraints(x);
    if ( h_norm == NOMAD::L2 )
        h = h.sqrt();
//cout << "h=" << h << " \n";    
}
    	/// Compute the estimation value of the unrelaxable constraints
    
    NOMAD::Double NOMAD::Quad_Model::estimate_unrelaxable_constraints (const NOMAD::Point  & x) const
    {   
        int i;
        std::set< const NOMAD::Eval_Point*> other_cache;
        other_cache=copy_the_cache();
        //int nb_voisins=leave_one_out_cross_validation_epanechnikov_unrelaxable_constraints(other_cache); 
	int nb_voisins=floor(sqrt(other_cache.size())); 
        //int nb_voisins=10; 
        //cout <<"nb_voisins="<< nb_voisins << "\n";
        int m  = static_cast<int>(_bbot.size());
        bool is_verifying_all_unrelaxable_constraints=true;
        bool booleen_test=true;
        NOMAD::Double estimation=1.0;
...
int compteur=0;
 ...
 {  is_verifying_all_unrelaxable_constraints=true;
 i=0;

    //cout <<*cur<< "\n";
    //cout <<"is_verifying_all_unrelaxable_constraints AVANT= "<<is_verifying_all_unrelaxable_constraints<< "\n";
     //cout<< (cur->get_bb_outputs())[0].is_defined() << " " << (cur->get_bb_outputs())[0]<< " " <<"coucou \n";

while(is_verifying_all_unrelaxable_constraints && i<m)
{ //cout<< "i= "<< i<< " ";
if ((cur->get_bb_outputs())[i].is_defined())
{
    if ( (_bbot[i] == NOMAD::EB || _bbot[i] == NOMAD::PEB_E) && (cur->get_bb_outputs())[i]>0)
    {
    //cout<< " EB constraint \n";
    is_verifying_all_unrelaxable_constraints=false;
    
    }

}
else {is_verifying_all_unrelaxable_constraints=false;
//cout<< "hidden constraint \n";
}
i++;
}
 //cout <<"is_verifying_all_unrelaxable_constraints= "<<is_verifying_all_unrelaxable_constraints<< "\n";
if (is_verifying_all_unrelaxable_constraints)  
{   //cout<<"realisable \n";
    un_voisin.origin=0; 
}
else 
...
un_voisin.distance=(*cur-x).norm();
un_voisin.eval_point=(*cur).get_bb_outputs();
//cout << un_voisin.distance << " "<< *cur <<"\n";
...
NOMAD::Double numerateur=0.0;
NOMAD::Double denominateur=0.0;

std::set<NOMAD::Voisin>::const_iterator it22 ;
std::set<NOMAD::Voisin>::const_iterator it222 ;
int counter1=0;
NOMAD::Double lambda=10;
    for ( it222 = voisins.begin() ; it222 != voisins.end() && counter1<nb_voisins; ++it222 )
    {   counter1++;
    if (counter1!=1)lambda=(*it222).distance;

    }
if (lambda==0) {lambda=1;}
int counter=0;
    for ( it22 = voisins.begin() ; it22 != voisins.end() && counter<nb_voisins; ++it22 )
    {   counter++;
        //std::cout << "distance="<<(*it22).distance << " ";
        //std::cout <<  "realisable= "<<(*it22).origin << "\n";
        //std::cout <<  "sorties= "<<(*it22).eval_point << "\n \n";
    //std::cout<<(*it22).distance<<"\n";
    if ((*it22).distance>0) 
    {   
        numerateur+=(*it22).origin*((1-(*it22).distance/lambda).pow2());
        denominateur+=(1-(*it22).distance/lambda).pow2();
    }   
...
{estimation=numerateur/denominateur;}
//cout << estimation << "\n ";
    return estimation;
    }
/// Compute the estimation value of a binary constraint

NOMAD::Double NOMAD::Quad_Model::estimate_binary_constraint (const NOMAD::Point  & x, int index) const
{
    //std::set<const NOMAD::Eval_Point*> other_cache=copy_the_cache (0);
    //bool booleen_a_la_con=true;
    std::set< const NOMAD::Eval_Point*> other_cache;
    other_cache=copy_the_cache();
    
    int n=-1; 
    //int nb_voisins=leave_one_out_cross_validation_epanechnikov(other_cache, index); // Will have to use LOOCV instead
    int nb_voisins=floor(sqrt(other_cache.size())); 
    //cout << nb_voisins<< " "<< index<<"\n";
    //int nb_voisins=10;
...
if((cur->get_bb_outputs())[index].is_defined()){
    compteur++;
 if ((cur->get_bb_outputs())[index]<=0) {un_voisin.origin=0; 
 
}

 else if ((cur->get_bb_outputs())[index]>0){un_voisin.origin=1;
...
std::set<const NOMAD::Eval_Point*> NOMAD::Quad_Model::copy_the_cache (void) const
...
/// Make the leave-one-out cross-validation with the epanechnikov kernel

int  NOMAD::Quad_Model::leave_one_out_cross_validation_epanechnikov (std::set<const NOMAD::Eval_Point*> my_cache, int index) const
{
    
     NOMAD::Double score_meilleur_k=1000000000.0; 
     int dimension;
     int dimension_carre;
  
    
    int meilleur_k=1;
    int taille= my_cache.size();
std::set<const NOMAD::Eval_Point*>::const_iterator yiyi ;
     yiyi = my_cache.begin();
           
       dimension=(**yiyi).size(); 
 
dimension_carre=dimension*dimension;
    // on teste pour beaucoup valeurs de k le modele knn
    for(int k=1; k<=dimension_carre;k++){
        //std::cout<< " k= " << k << "\n \n"; cin.get();
    //std::cout<< " ca, ca, c'est vraiment k= " << k << "\n \n";  cin.get();
     NOMAD::Double score_k=0.0;


      
     std::set<const NOMAD::Eval_Point*>::const_iterator yoyo ;
       for ( yoyo = my_cache.begin() ; yoyo != my_cache.end(); ++yoyo )
        {
              if(((*yoyo)->get_bb_outputs())[index].is_defined()){
              std::set<const NOMAD::Eval_Point*>::const_iterator yaya ;
              
              NOMAD::Double knn_score_epanechnikov=0.5;
              std::set <NOMAD::Voisin> voisins;   
                NOMAD::Voisin un_voisin; 
                NOMAD::Voisin nouveau_voisin; 


                 
              
              for ( yaya = my_cache.begin() ; yaya != my_cache.end(); ++yaya ){
                                  if (yoyo==yaya) { 
                        
                     }
                                  else{
                  if(((*yaya)->get_bb_outputs())[index].is_defined()){
                  
                    //calculer les distances et associer les classes

                     {
                           un_voisin.distance=((**yoyo)- (**yaya)).norm(); 
                           if (((*yaya)->get_bb_outputs())[index]<=0){
                               un_voisin.origin=0;
                            }
                           else if (((*yaya)->get_bb_outputs())[index]>0){
                               un_voisin.origin=1;
                            }
                           voisins.insert(un_voisin);
                    }  
...
int counter1=0;
NOMAD::Double lambda=10;
    for ( it222 = voisins.begin() ; it222 != voisins.end() && counter1<k; ++it222 )
    {   counter1++;
    if (counter1!=1)lambda=(*it222).distance;
...
if (lambda==0) {lambda=1;}
int counter=0;
    for ( it22 = voisins.begin() ; it22 != voisins.end() && counter<k; ++it22 )
    {   counter++;
        //std::cout << (*it22).distance << " ";
        //std::cout << (*it22).origin << "\n ";
    //std::cout<<(*it22).distance<<"\n";
    ...
    if (denominateur==0) {knn_score_epanechnikov=0.5;} else {knn_score_epanechnikov=numerateur/denominateur;}
    

       
    }
                  if ((*yoyo)->is_feasible(0)){
                               score_k+=knn_score_epanechnikov.pow2();
                            }
                           else {
                               score_k+=(1-knn_score_epanechnikov).pow2();
                            }
         
          }
              
       }
    //   std::cout<<  k << " a un score_k  " << score_k << "\n \n"; 
         
//mettre a jour score_meilleur_k
     //std::cout<< " k " << k << "\n \n"; 
     //std::cout<< " meilleur_k  " << meilleur_k << "\n \n"; 
     //std::cout<< " score_meilleur_k  " << score_meilleur_k << "\n \n"; 
     //std::cout<< " score_k  " << score_k << "\n \n"; 
     if (score_k<score_meilleur_k) {score_meilleur_k=score_k; meilleur_k=k;}
    }
//std::cout<< " meilleur_k  " << meilleur_k << "\n \n";  cin.get();
    return (meilleur_k);
}
int  NOMAD::Quad_Model::leave_one_out_cross_validation_epanechnikov_unrelaxable_constraints (std::set<const NOMAD::Eval_Point*> my_cache) const
{
    int i;
     NOMAD::Double score_meilleur_k=1000000000.0; 
     int dimension;
     int dimension_carre;
     int m  = static_cast<int>(_bbot.size());
     bool is_verifying_all_unrelaxable_constraints_yoyo=true;
     bool is_verifying_all_unrelaxable_constraints_yaya=true;
 ...
 for(int k=1; k<=dimension_carre;k++){
    ...
    //std::cout<< " ca, ca, c'est vraiment k= " << k << "\n \n";  cin.get();
     ...
     std::set<const NOMAD::Eval_Point*>::const_iterator yoyo ;
       for ( yoyo = my_cache.begin() ; yoyo != my_cache.end(); ++yoyo )
        {
              is_verifying_all_unrelaxable_constraints_yoyo=true;
              std::set<const NOMAD::Eval_Point*>::const_iterator yaya ;
              ...
              for ( yaya = my_cache.begin() ; yaya != my_cache.end(); ++yaya )
               {    if (yoyo==yaya) { 
                    ...
                    //calculer les distances et associer les classes
                     ...
                     else{ is_verifying_all_unrelaxable_constraints_yaya=true;
                           un_voisin.distance=((**yoyo)- (**yaya)).norm();
                           i=0;
                     while(is_verifying_all_unrelaxable_constraints_yaya && i<m){
                            //cout<< "i= "<< i<< " ";
if (((*yaya)->get_bb_outputs())[i].is_defined())
{
    if ( (_bbot[i] == NOMAD::EB || _bbot[i] == NOMAD::PEB_E) && ((*yaya)->get_bb_outputs())[i]>0)
    {
    //cout<< " EB constraint \n";
    is_verifying_all_unrelaxable_constraints_yaya=false;
    
    }

}
else {is_verifying_all_unrelaxable_constraints_yaya=false;
//cout<< "hidden constraint \n";
}
i++;
}
                           if(is_verifying_all_unrelaxable_constraints_yaya) {un_voisin.origin=0;}
...
i=0;       
while(is_verifying_all_unrelaxable_constraints_yoyo && i<m){
                            //cout<< "i= "<< i<< " ";
if (((*yoyo)->get_bb_outputs())[i].is_defined())
{
    if ( (_bbot[i] == NOMAD::EB || _bbot[i] == NOMAD::PEB_E) && ((*yoyo)->get_bb_outputs())[i]>0)
    {
    //cout<< " EB constraint \n";
    is_verifying_all_unrelaxable_constraints_yaya=false;
...
i++;
                  ...
                  if (is_verifying_all_unrelaxable_constraints_yoyo){
                               score_k+=knn_score_epanechnikov.pow2();
                            }
                           else {
                               score_k+=(1-knn_score_epanechnikov).pow2();
